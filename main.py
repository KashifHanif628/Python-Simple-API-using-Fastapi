...
# we will buld two simple get endpoints.
# side_hustles: it will tell us which things we can do.
# money_quotes: 
...
#decorator which is a rout handler and two get api endpoints.
@app.get("/side_hustles")
def get_side_hustles(apikey: str): # we have passed the key so no body can access our api without this parameter key.
    if apikey != "123456": # key 
...
def get_money_quotes(apikey: str): # we have passed the key so no body can access our api without this parameter key.
    if apikey != "123456": # key
        return {"error": "Invalid API key"}
